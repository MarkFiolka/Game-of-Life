...
public class ScrollLogic
{
    ...
    private readonly PatternPreviewBoard previewBuilder;
    ...
    public ScrollLogic(ScrollView scrollView, PatternList patternList, PatternPreviewBoard previewBuilder)
    {
    ...
    public void PopulateButtons()
    {
        ...
        for (int i = -objCount; i < objCount * 2; i++)
        {
            ...
            button.clicked += () => CenterButton(button, capturedIndex);
    ...
    private void CenterButton(VisualElement button, int index)
    {
        ...
        var pattern = patternList.getObjectAtIndex(index) as Pattern;
        if (pattern != null)
        {
            previewBuilder.ShowPattern(pattern);
        }
        else
        {
            Debug.LogWarning($"Pattern at index {index} is null or not a Pattern ScriptableObject.");
        }
    }
    ...
    public Pattern GetSelectedPattern()
    {
        ...
        int index = buttons.IndexOf(selectedButton) % objCount;
        return patternList.getObjectAtIndex(index) as Pattern;
    }
